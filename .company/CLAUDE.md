...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"🚀 [部署名]: [タスク概要] 開始\",\"description\":\"[リクエスト内容の要約]\",\"color\":5763719,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"✅ [部署名]: [タスク概要] 完了\",\"description\":\"保存先: [path]\",\"color\":5763719,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"⏰ [タスク名]\",\"description\":\"[補足メモ]\",\"color\":3447003,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"🌅 朝のブリーフィング YYYY-MM-DD\",\"description\":\"[タスク数・主要ニュース見出し]\",\"color\":3447003,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"📋 処理計画: キュータスク X件\",\"description\":\"[タスク一覧と担当部署]\",\"color\":10181046,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"🚀 秘書室: キュータスク X件 処理開始\",\"description\":\"[タイトル一覧]\",\"color\":5763719,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
| `idea_development` | 実装確定アイデアの開発準備。以下を順番に作成: ① 要件定義書（目的・対象ユーザー・機能要件・非機能要件） ② 詳細設計書（画面設計・DB設計・API設計・技術スタック） ③ Claude Code MVP用プロンプト（このプロンプトをClaude Codeに渡せばMVPが作れるレベルで記述） **実装・コーディング自体は行わない。** | 適切な事業部フォルダ |
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"embeds\":[{\"title\":\"✅ [部署名]: [タスクタイトル] 完了\",\"description\":\"保存先: [path]\",\"color\":5763719,\"footer\":{\"text\":\"渡邊カンパニー 秘書室\"}}]}"
```
...
保存先は `3125[事業部名]/YYYY-MM-DD-[プロジェクト名]-開発仕様書.md`
