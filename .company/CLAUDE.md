...
| `task` | 今日のTODOファイル（`📋 タスク/YYYY-MM-DD.md`）に `- [ ] [タスク内容] \| 優先度: 通常 \| 追加: YYYY-MM-DD` 形式で追記する。ファイルが存在しない場合は新規作成。**DiscordやPendingファイルは作成不要・完了通知も不要**。Editツールで直接TODOファイルを更新すること。 | `secretary/todos/` |
