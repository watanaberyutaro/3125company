...
`- [x] 振り分け` がチェックされたファイルのみ移動対象。振り分け先は他のチェックで決まる（優先度: 取り組み必要 > お気に入り > 閲覧済み）:

| 振り分け | 追加チェック | 移動先 |
|---------|------------|--------|
| `- [x]` | なし | `target_folder/` |
| `- [x]` | `- [x] 閲覧済み` | `target_folder/_archive/` |
| `- [x]` | `- [x] お気に入り` | `target_folder/_お気に入り/` |
| `- [x]` | `- [x] 取り組み必要` | `target_folder/_取り組み必要/` |
| `- [ ]` | 何でも | 受信トレイに留まる |
...
| `minutes` | 議事録要約 | `00_受信トレイ/`（target: 役員トーク→アイゼン/役員トーク、商談→アイゼン/営業情報） |
